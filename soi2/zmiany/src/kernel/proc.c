	...
	message *m_ptr; /* pointer to message buffer */
{
	...
	if (vlo < caller_ptr->p_map[D].mem_vir || vlo > vhi
			|| vhi
					>= caller_ptr->p_map[S].mem_vir
							+ caller_ptr->p_map[S].mem_len)
		return (EFAULT);
#else
	...
	register struct proc *rp; /* this process is now runnable */
{
	...
	if (rdy_head[USER_Q]->proc_type > rp->proc_type) {
	...
	tmp = rdy_head[USER_Q];
	tmp1 = tmp->p_nextready;

	if (rp->proc_type == TYPE1) {
		while ((tmp1 != NIL_PROC) && (tmp1->proc_type == TYPE1)) {
			tmp = tmp1;
	...
	else if (rp->proc_type == TYPE2) {
		/*rp->curr_pri = rp->base_pri;*/

		while ((tmp1 != NIL_PROC) && (tmp1->proc_type < TYPE2)) {
			tmp = tmp1;
			tmp1 = tmp1->p_nextready;
		}
		while ((tmp1 != NIL_PROC) && (rp->curr_pri < tmp1->curr_pri)
				&& (tmp->proc_type == TYPE2)) {
			tmp = tmp1;
			tmp1 = tmp1->p_nextready;
		}

		if(tmp==rdy_head[USER_Q]){
			if(rp->curr_pri<tmp->curr_pri){
				rp->p_nextready = rdy_head[USER_Q];
				rdy_head[USER_Q] = rp;
				return;
			}
			else{
				rp->p_nextready= rdy_head[USER_Q]->p_nextready;
	...
	else if (rp->proc_type == TYPE3) {
		while ((tmp1 != NIL_PROC) && (tmp1->proc_type <= TYPE2)) {
			tmp = tmp1;
			tmp1 = tmp1->p_nextready;
		}
		while ((tmp1 != NIL_PROC) && (rp->base_pri > tmp1->base_pri)) {
			tmp = tmp1;
			tmp1 = tmp1->p_nextready;
		}

		if (tmp == rdy_head[USER_Q] && tmp->base_pri > rp->base_pri) {
			rp->p_nextready = rdy_head[USER_Q];
			rdy_head[USER_Q] = rp;
			return;
		}
...
PRIVATE void sched() {
	...
	nxt = rdy_head[USER_Q]->p_nextready;
	/* One or more user processes queued. */

	if (rdy_head[USER_Q]->proc_type == TYPE1) {
		while ((tmp1 != NIL_PROC) && (tmp1->proc_type == TYPE1)) {
			tmp = tmp1;
			tmp1 = tmp1->p_nextready;
		}
		if (tmp == rdy_head[USER_Q]) {
			...
			if (tmp1 == NIL_PROC) {
				...
				rdy_head[USER_Q]->p_nextready = tmp1;
	...
	else if (rdy_head[USER_Q]->proc_type == TYPE2) {
		rdy_head[USER_Q]->curr_pri = rdy_head[USER_Q]->base_pri;
		...
		while ((tmp1 != NIL_PROC) && (tmp1->proc_type == TYPE2)) {
			if (tmp1->curr_pri < MAX_PRI)
				tmp1->curr_pri = tmp1->curr_pri + 1;
			tmp = tmp1;
			tmp1 = tmp1->p_nextready;
		}
		...
		tmp1 = tmp->p_nextready;
		while ((tmp1 != NIL_PROC) && (tmp1->proc_type <= TYPE2)
				&& (tmp1->curr_pri > rdy_head[USER_Q]->base_pri)) {
			tmp = tmp1;
			tmp1 = tmp1->p_nextready;
		}
		if (tmp == rdy_head[USER_Q]) {
